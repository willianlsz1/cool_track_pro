...
describe('orcamentoFollowUp', () => {
  ...
  it('habilita follow-up apos X dias em status enviado', () => {
  ...
  it('nao habilita follow-up para orcamento aprovado', () => {
    const now = new Date('2026-04-28T00:00:00.000Z').getTime();
    const sentAt = new Date(now - 10 * 24 * 60 * 60 * 1000).toISOString();
    const meta = getFollowUpMeta({ status: 'aprovado', enviadoEm: sentAt }, now);
    expect(meta.shouldShow).toBe(false);
  });

  it('faz parse estavel de data-only para follow-up sem drift de fuso', () => {
