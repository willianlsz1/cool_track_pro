...
const mocks = vi.hoisted(() => ({
  ...
  setState: vi.fn((updater) => updater({ registros: [], equipamentos: [], tecnicos: [] })),
  goTo: vi.fn(),
...
vi.mock('../../core/photoStorage.js', () => ({
  uploadPendingPhotos: vi.fn(async (photos) => ({ photos, failedCount: 0 })),
}));
...
describe('regression: clear registro always exits edit mode', () => {
  ...
  it('A) cancelar limpa sessionStorage e próxima entrada fica modo novo', async () => {
    const { loadRegistroForEdit, clearRegistro } = await import('../../ui/views/registro.js');
  ...
  it('B) salvar edição limpa sessionStorage', async () => {
  ...
  it('C) saída sem salvar limpa sessionStorage (regressão preventiva)', async () => {
