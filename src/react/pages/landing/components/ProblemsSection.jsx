 ...
 * aparece inline no mobile e no painel lateral em desktop com a `solution` correspondente.
 *
    ...
    >
      ...
      <div className="tw-relative tw-max-w-[1280px] tw-mx-auto tw-px-6">
        ...
        <div
          className={
            activeProblem
              ? 'tw-grid tw-grid-cols-1 lg:tw-grid-cols-[minmax(0,1fr)_minmax(320px,420px)] tw-gap-5 lg:tw-gap-8 tw-items-start'
              : 'tw-block'
          }
        >
          <div
            className={
              activeProblem
                ? 'tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4 sm:tw-gap-5'
                : 'tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 lg:tw-grid-cols-3 tw-gap-4 sm:tw-gap-5'
            }
          >
            {problems.map((p) => {
              ...
              return (
                <Fragment key={p.id}>
                  ...
                  {isActive ? <SolutionPanel problem={p} variant="inline" /> : null}
                </Fragment>
              );
            })}
          </div>

          {activeProblem ? <SolutionPanel problem={activeProblem} variant="side" /> : null}
        </div>
      </div>
    </section>
  );
...
function ProblemCard({ problem, isActive, onClick }) {
  const IconComp = PROBLEM_ICONS[problem.id];
  ...
  return (
    <button
      ...
      style={{
        ...
        border: isActive ? '1px solid rgba(64,196,255,0.55)' : '1px solid rgba(120,170,230,0.18)',
        boxShadow: isActive
    ...
    >
      ...
      <h3 className="tw-text-base tw-font-bold tw-mb-1.5 tw-text-white">{problem.title}</h3>
      <p className="tw-text-sm tw-text-[#a8bcd9] tw-leading-[1.5]">{problem.description}</p>
    </button>
  );
}

function SolutionPanel({ problem, variant }) {
  const variantClasses =
    variant === 'inline'
      ? 'solution--inline tw-flex lg:tw-hidden tw-col-span-full tw-relative tw-mt-3 tw-mb-3.5'
      : 'solution--side tw-hidden lg:tw-flex lg:tw-sticky lg:tw-top-8';

  return (
    <aside
      aria-live="polite"
      role="complementary"
      aria-label="Como o CoolTrack resolve este problema"
      className={`tw-rounded-2xl tw-px-6 tw-py-5 tw-flex-col sm:tw-flex-row tw-items-start tw-gap-4 ${variantClasses}`}
      style={{
