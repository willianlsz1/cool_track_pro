# app-v2 - CP-44 remocao do CSS legado layout

## Objetivo

Remover a folha legada `src/assets/styles/layout.css`, que nao e carregada pelo
entrypoint principal app-v2 e permanecia como shell/grid visual do v1.
...
```bash
rg -n "layout\\.css|assets/styles/layout\\.css" index.html src public e2e vite.config.js package.json
```

Achados:

- `index.html` nao referencia `layout.css`.
- `src/app-v2/**` nao importa `layout.css`.
- `public/`, `e2e/`, `vite.config.js` e `package.json` nao referenciam
  `layout.css`.
- A unica ocorrencia runtime fora do proprio CSS era um comentario em
  `src/assets/styles/tokens.css`.

## Arquivos alterados

- Removido: `src/assets/styles/layout.css`
- Atualizado: `src/__tests__/legacyV1RemovalContracts.test.js`
- Atualizado: `src/assets/styles/tokens.css`
- Atualizado: `docs/rewrite/app-v2-remocao-v1-vestigios-plano.md`

## Fora do escopo

- Nao remover `components.css`, `redesign.css`, `tokens.css` ou CSS de
  Equipamentos.
