...
| Dashboard                              | KPIs, proxima acao, ultimo servico e resumo mensal.  | Hero protegido por teste legado, empty/onboarding, overflow banner, charts, cards Pro, alertas mini, criticos, recentes, rascunho. | Fallbacks dos blocos React (`_renderNextActionCard`, `_renderLastServiceCard`, `_renderMonthView`) removidos; `#dash-hero` mapeado e protegido para futura ilha. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho e equipamentos.    | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Alto.       | Migrar `dash-hero` em PR pequeno; depois revisar secoes secundarias do Dashboard.                      |
...
- `#dash-hero` foi mapeado e protegido por `src/__tests__/dashboardLegacyHero.test.js`; `DASHBOARD_PUBLIC_CLASSES` agora centraliza as classes publicas do hero que uma futura ilha deve preservar.
...
- Escopo: migrar primeiro `#dash-hero`, que ja tem contratos protegidos; depois empty state, alertas mini, criticos e recentes em ilhas pequenas ou uma ilha read-only por secao.
