...
| -------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------ |
| Alertas                                | Tela inteira em `#view-alertas`.                     | Adapter le `state`, monta view model, skeleton e unmount.                                                        | `ensureAlertasShell()` pode ser redundante porque o shell ja declara `#alertas-contextual` e `#lista-alertas`.                                          | `data-action`, `data-id`, `data-nav` emitidos pelos cards/empty state.                                      | `alert-*`, `alertas-*`, `empty-state`, `btn`.                                           | Baixo/medio. | Provar shell fixo e remover fallback `innerHTML` de `alertas.js`.                                      |
...
| Dashboard                              | KPIs, proxima acao, ultimo servico e resumo mensal.  | Hero, empty/onboarding, overflow banner, charts, cards Pro, alertas mini, criticos, recentes, rascunho.          | Fallbacks DOM em `_renderNextActionCard`, `_renderLastServiceCard` e `_renderMonthView` podem ser removiveis se os roots e view model forem garantidos. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho e equipamentos.    | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Alto.        | Migrar `dash-hero` como ilha read-only ou remover fallbacks dos blocos ja migrados em PR proprio.      |
...
| Relatorio                              | Hero, controles/filtros e cards.                     | PDF, WhatsApp, PMOC, quota, assinatura viewer, navegacao e company PMOC block.                                   | `renderModeSegment()` e `renderFilterChips()` sao fallbacks quando `#rel-controls-root` nao existe; hoje o root existe no shell.                        | `data-action`, `data-rel-action`, `data-nav`, `data-id`, `data-view-mode`, `data-tier`.                     | `rel-*`, `servicos-toggle*`, `pmoc*`, quota/PDF.                                        | Medio/alto.  | Migrar bloco `#rel-company-pmoc-slot` ou remover fallbacks de controles.                               |
...
- `ensureAlertasShell()` em `src/ui/views/alertas.js`, desde que o shell continue declarando `#alertas-contextual` e `#lista-alertas`.

Media seguranca, apos prova de root sempre presente e testes de lifecycle:

- Fallbacks DOM de dashboard para blocos ja migrados: partes de `_renderNextActionCard`, `_renderLastServiceCard` e `_renderMonthView` que so rodam quando nao existe root/model.
- Fallback de `relatorio.js` quando `#rel-controls-root` nao existe: `renderModeSegment()` e `renderFilterChips()`.
