...
| Equipamentos                           | Lista flat em `#lista-equip`.                                                                                              | Hero, filtros e contexto protegidos por `equipamentosLegacyHeroFiltersContext.test.js`; setores, detalhe/modal, fotos, CRUD, nameplate, plano e storage seguem legados. | Renders antigos de card/lista podem estar duplicados entre `equipmentCards.js` e React list, mas ainda usados em setores/detalhe.                                                                                                                                                | `data-action`, `data-id`, `data-setor-id`, `data-cliente-id`, callbacks de fotos/PMOC.                           | `equip-*`, `setor-*`, `eq-*`, `_equip-hero.css`, `_setor-card.css`, `_setor-modal.css`. | Alto.  | Migrar `#equip-hero`, `#equip-filters` e `#equip-context-chip` para ilha separada mantendo setores/detalhe/fotos/CRUD legados. |
...
- Status: preparado por `src/__tests__/equipamentosLegacyHeroFiltersContext.test.js`, sem React novo nesta etapa.
...
- Testes atuais: contratos legados de hero, filtros, busca, toolbar e contexto; proxima fatia deve adicionar island nova, legacy handlers dos filtros e E2E equipamentos -> inicio -> equipamentos.
- Risco: medio/alto.
