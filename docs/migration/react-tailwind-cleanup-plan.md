...
| Dashboard                              | Hero, KPIs, proxima acao, ultimo servico e resumo mensal. | Empty/onboarding, overflow banner, charts, cards Pro, alertas mini, criticos, recentes, rascunho.                | Fallbacks dos blocos React (`_renderNextActionCard`, `_renderLastServiceCard`, `_renderMonthView`) removidos; `#dash-hero` delega para `dashboardHeroIsland`; alertas mini/criticos/recentes protegidos por contrato legado. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho e equipamentos.    | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Alto.       | Migrar alertas mini/criticos/recentes em ilha read-only pequena; manter charts/onboarding/header fora do escopo. |
...
- Escopo: depois do `#dash-hero`, migrar alertas mini, criticos e recentes em ilhas pequenas ou uma ilha read-only por secao; empty state pode ficar para uma fatia separada.
