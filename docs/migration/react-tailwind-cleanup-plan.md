...
| Dashboard                              | Hero, KPIs, proxima acao, ultimo servico, resumo mensal e read-only blocks. | Empty/onboarding, overflow banner, charts, cards Pro, rascunho.                                                  | Fallbacks dos blocos React (`_renderNextActionCard`, `_renderLastServiceCard`, `_renderMonthView`) removidos; `#dash-hero` delega para `dashboardHeroIsland`; alertas mini/criticos/recentes delegam para `dashboardReadOnlyBlocksIsland`; cards Pro e rascunho estao protegidos por contrato legado. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho e equipamentos.    | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Alto.       | Migrar cards Pro/rascunho ou proteger empty/onboarding em fatias separadas; manter charts/header fora do escopo. |
...
- `#dash-alerts-section`, `#dash-critical-section`, `#dash-criticos-section` e `#dash-recentes-section` foram migrados para `src/react/entrypoints/dashboardReadOnlyBlocksIsland.jsx`; `src/__tests__/dashboardReadOnlyBlocksIsland.test.jsx` cobre lifecycle, contratos e XSS. Cards Pro continuam legados.
- `#dash-pro-ops-row` e o card `.dash__continue-card` de rascunho continuam legados e estao protegidos por `src/__tests__/dashboardLegacyProDraftContracts.test.js`, cobrindo Free/Plus/Pro, `continue-draft`, `discard-draft`, `data-nav="clientes"` e XSS.
...
### PR 4b: Dashboard cards Pro e rascunho legados

- Status: protegido por contrato legado; ainda sem React.
...
- Testes: `dashboardLegacyProDraftContracts.test.js`.
- Risco: medio, porque `_renderProCards()` ainda usa HTML legado sanitizado e `_renderContinueDraftCard()` depende de `sessionStorage`.
- Criterio: migrar ou limpar em PR proprio sem quebrar `data-action`, `data-nav`, `data-id` e `data-tier`.
