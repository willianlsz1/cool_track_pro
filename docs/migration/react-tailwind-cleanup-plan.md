...
| Dashboard                              | Hero, KPIs, proxima acao, ultimo servico, resumo mensal, read-only blocks, cards Pro e rascunho. | Empty/onboarding, overflow banner, charts.                                                                       | Fallbacks dos blocos React (`_renderNextActionCard`, `_renderLastServiceCard`, `_renderMonthView`, `_renderProCards`, `_renderContinueDraftCard`) removidos; `#dash-empty`, `#dash-onboarding` e `#dash-overflow-banner` agora estao protegidos como contratos legados. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho, onboarding e upgrade. | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Medio/alto. | Migrar ou limpar empty/onboarding/overflow em fatia separada; depois proteger charts/header.           |
...
- `#dash-empty`, `#dash-onboarding` e `#dash-overflow-banner` foram protegidos por `src/__tests__/dashboardLegacyOnboardingEmptyOverflow.test.js`; os contratos atuais de empty state, checklist/banner de onboarding e overflow banner estao centralizados em `dashboardContracts.js`.
...
- Escopo: alertas mini, criticos e recentes em uma ilha read-only; empty state fica para uma fatia separada.
...
- Fora do escopo: plano/paywall real, checkout, charts, onboarding, overflow banner e header global.
...
- Status: contratos legados protegidos; ainda nao migrado para React.
- Escopo atual: `#dash-empty`, `#dash-onboarding`, checklist/banner de onboarding e `#dash-overflow-banner`.
- Fora do escopo: charts, header global, plano/paywall real e checkout.
- Testes: `dashboardLegacyOnboardingEmptyOverflow.test.js`.
- Risco: medio, porque `OnboardingChecklist` usa storage/telemetria e `OverflowBanner` ainda pode abrir modal one-shot no fluxo real.
- Criterio para proxima etapa: migrar somente o bloco visual/contratos para ilha React ou provar obsolescencia de partes especificas sem tocar em charts/header.
...
O proximo PR recomendado agora e decidir a fatia visual final do Dashboard: migrar somente `#dash-empty`, `#dash-onboarding` e `#dash-overflow-banner` para uma ilha React controlada, ou manter esses blocos legados e proteger `charts` como contrato antes de qualquer limpeza.

Motivo: os blocos ja migrados do Dashboard nao mantem fallback DOM morto, e empty/onboarding/overflow agora tem teste de contrato. A menor proxima fatia util e transformar essa cobertura em ilha controlada ou fechar a decisao de manter o legado por causa de storage/telemetria/modal one-shot.
