...
| Dashboard                              | Hero, KPIs, proxima acao, ultimo servico e resumo mensal. | Empty/onboarding, overflow banner, charts, cards Pro, alertas mini, criticos, recentes, rascunho.                | Fallbacks dos blocos React (`_renderNextActionCard`, `_renderLastServiceCard`, `_renderMonthView`) removidos; `#dash-hero` agora delega para `dashboardHeroIsland`. | `data-action`, `data-nav`, `data-id` para registro, modal, historico, clientes, rascunho e equipamentos.    | `dash-*`, `critical-*`, `recent-*`, `chart-*`, onboarding/overflow.                     | Alto.       | Proteger e migrar empty/alertas mini/criticos/recentes em fatias pequenas; manter charts/onboarding/header fora do escopo. |
...
- Escopo: depois do `#dash-hero`, migrar empty state, alertas mini, criticos e recentes em ilhas pequenas ou uma ilha read-only por secao.
