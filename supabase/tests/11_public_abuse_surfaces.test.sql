-- ============================================================
-- Test: CP-G superficies publicas anti-abuso
--
-- Valida que:
--   - analytics_events aceita evento anonimo legitimo, mas nao user_id forjado.
--   - feedback aceita envio legitimo, mas nao user_id forjado.
--   - registro-fotos tem bucket privado com limite versionado.
--   - helpers/policies de Storage preservam fotos legitimas e bloqueiam path
--     de outro usuario ou scope inesperado.
-- ============================================================
...
begin
  ...
  if to_regprocedure('public.can_write_registro_signature_storage_object(text,text,uuid)') is not null then
    raise exception 'FAIL: helper legado de assinatura de registro deveria estar aposentado';
  end if;
  ...
  if not public.can_write_registro_fotos_storage_object(
    ...
    raise exception 'FAIL: path legado assinatura.png deve seguir contrato geral de foto de registro';
  end if;
...
\echo 'ok 1 - public abuse surfaces hardening'
